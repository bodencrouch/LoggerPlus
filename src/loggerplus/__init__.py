...
from loggerplus._misc import ensure_directory_exists

from loggerplus.error_handling import format_exception_with_variables
    ...
    def advanced_configure_logger(
        ...
        *,
        ...
        if not logger.handlers:
            from loggerplus._misc import is_debug_mode
            ...
            if cur_process.name == "MainProcess":
                ...
                everything_log_file = "debug_loggerplus.log"
                info_warning_log_file = "loggerplus.log"
                error_critical_log_file = "errors_loggerplus.log"
                exception_log_file = "exception_loggerplus.log"
            else:
                log_dir = get_log_directory(subdir=f"logs/{cur_process.pid}")
                everything_log_file = f"debug_loggerplus_{cur_process.pid}.log"
                info_warning_log_file = f"loggerplus_{cur_process.pid}.log"
                error_critical_log_file = f"errors_loggerplus_{cur_process.pid}.log"
                exception_log_file = f"exception_loggerplus_{cur_process.pid}.log"
